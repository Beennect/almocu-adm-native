...
export default observer(function AddItemScreen() {
  ...
  const handleAddIngredient = () => {
    ...
    const isUnidades = stockIng?.unit === 'Unidades' || !stockIng;

    if (!newIngredient.name || (isUnidades && !newIngredient.quantity)) {
      toastStore.show(isUnidades ? "Selecione um ingrediente e informe a quantidade." : "Selecione um ingrediente.", "error");
      return;
    }
    ...
    if (isUnidades) {
      ...
      if (isNaN(qty) || qty <= 0) {
        toastStore.show("A quantidade deve ser maior que 0.", "error");
        return;
      }
    }
  ...
  const handleAddItem = () => {
    if (!formData.name || !formData.value || !formData.category) {
      toastStore.show("Por favor, preencha o nome, valor e categoria.", "error");
      return;
    }

    if (ingredients.length === 0) {
      toastStore.show("Adicionar pelo menos um ingrediente ao item.", "error");
      return;
    }

    const servesVal = parseInt(formData.serves);
    if (isNaN(servesVal) || servesVal <= 0) {
      toastStore.show("A quantidade de pessoas deve ser maior que 0.", "error");
      return;
    }
    ...
    try {
      ...
      if (isEditing) {
        dataStore.updateItem(params.id as string, itemData);
        toastStore.show("Item atualizado com sucesso!", "success");
      } else {
        dataStore.addItem(itemData);
        toastStore.show("Item adicionado ao cardápio!", "success");
      }
      
      router.back();
    } catch (err: any) {
      toastStore.show(err.message || "Erro ao processar item.", "error");
    }
  };
